...
import type {
    IFormProps,
    IInputBook,
} from '@/shared/shared-forms/shared-forms';
import './Form.scss';
import './FormMedia.scss';

const Form = ({ id, text, format = null, index = null }: IFormProps) => {
    // используем reactHookForm
    const { register, handleSubmit, formState, reset } = useForm<IInputBook>({
        mode: 'onChange',
    });
    ...
    useEffect(() => {
        ...
        if (mutationBook.isError || mutationWebinar.isError) {
            setUserNotification(
                <p className="roboto-regular error_msg">
            ...
            setUserNotification(
                <p className="roboto-regular success_msg">
        ...
        mutationBook.isError,
        mutationWebinar.isError,
        mutationBook.isPending,
        mutationWebinar.isPending,
        mutationBook.isSuccess,
        mutationWebinar.isSuccess,
    ]);
    ...
    return (
        <>
            ...
            {emailError ? (
                <p className="roboto-regular error_msg">{emailError}</p>
            ) : null}
            {userNotification}
        </>
    );
